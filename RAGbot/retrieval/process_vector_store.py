...
class ProcessVectorStore:
    ...
    def ensure_catalog(self, catalog: Sequence[Dict[str, Any]], *, refresh: bool = False,
        ...
        try:
            ...
            missing = [(pair, text) for pair, text in documents.items() if refresh or pair not in existing]
            inserted = 0
    ...
    def _embed(self, texts: Sequence[str]) -> Sequence[Sequence[float]]:
        ...
        with OpenAI(
            ...
            timeout=90,
            max_retries=0,
            # Preserve the existing embedding client's proxy behavior.
