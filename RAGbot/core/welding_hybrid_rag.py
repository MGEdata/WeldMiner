...
class WeldingAgenticRAG:
    ...
    def query(self, question: str, top_k: Optional[int] = None) -> Dict[str, Any]:
        ...
        if not hits:
            return {
                "answer": "No sufficiently relevant data was found in the current welding database.",
                "error": None,
                **shared,
            }
        if not self.llm_enabled:
            return {
                "answer": "Structured retrieval is complete, but no language model is configured. Please review the retrieved evidence.",
        ...
        try:
            content = self._invoke_qwen(prompt, max_tokens=1800)
            return {"answer": content, "error": None, **shared}
        except Exception as exc:
            return {
                "answer": "Answer generation failed, but structured retrieval is complete. Please review the retrieved evidence.",
                "error": str(exc),
                **shared,
            }
