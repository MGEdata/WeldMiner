...
SUPPORTED_INPUT_EXTENSIONS = {".xml", ".pdf", ".md", ".markdown", ".txt"}
...
def process_xml_file(xml_path: Path) -> Optional[Tuple[str, str]]:
    """
    Parse XML file and return:
    - document folder id (derived from extracted DOI)
    - consolidated text content for the workflow
    """
    try:
    ...
    try:
        article_info = ElsevierXmlReader(str(xml_path)).dic()
    except Exception as e:
        print(f"Failed to parse XML {xml_path.name}: {e}")
        return None

    if not isinstance(article_info, dict):
        return None

    extracted_doi = str(article_info.get("doi") or xml_path.stem).strip()
    ...
    tables_text_parts = []
    tables = _extract_tables_from_xml(xml_path, extracted_doi)
    ...
    if not all_infos:
        print(f"No usable XML content: {xml_path.name}")
        return None
