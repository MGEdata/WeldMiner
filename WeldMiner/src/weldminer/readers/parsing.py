...
def _save_markdown(source: Path, content: str) -> Path:
    ...
    # Include the source suffix to distinguish paper.pdf from paper.xml.
...
def parse_xml(path: str | Path) -> Path:
    """Parse one XML document and its tables; return adjacent <filename>.xml.md."""
    source = _source_file(path, '.xml')
    from .parser import load_document_input
