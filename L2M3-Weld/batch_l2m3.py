...
OVERWRITE = False
DRY_RUN = False  # True = parse only; no API requests.
...
def build_processor(args, llm):
    ...
    agent = LLMMiner.from_llm(llm, llm) if llm is not None else None
    def process(file, folder):
        ...
        if args.dry_run: return {'input_count': len(journal.elements), 'doi': journal.doi}
...
def run_batch(args=None):
    ...
    for index, file in enumerate(files, 1):
        ...
        marker = folder / ('parse_stats.json' if args.dry_run else 'stats.json')
...
def save_summary(output, run_id, rows, args, started):
    ...
    write_json(output / f'summary-{run_id}.json', {
        ...
        'thinking': args.thinking, 'dry_run': args.dry_run, 'temperature': args.temperature,
