PROPERTY_CATEGORIZE = """Select only explicitly present welding property keys from these definitions:
{{ explanation }}
Use the current configured property keys. Do not mistake σ HE or δ HE for yield strength.
Interpass/PWHT temperatures are not Charpy test temperatures. CTOD, yield ratio,
reduction in area and hydrogen embrittlement indices belong to etc, not yield_strength.
Do not label pure chemistry as a mechanical property. Standard minimum/maximum limits
are not measured specimens. Inspect headers and cells even when the caption is generic.
The following examples use representative real welding tables.
Return only a Python/JSON list with no prefix or code fence, as required by this table parser.
...
Notes: Table 5
List: ["tensile_strength", "elongation", "etc"]
...
List: ["yield_strength", "tensile_strength", "elongation", "etc"]
...
Notes: Table 7
List: ["impact_energy", "charpy_impact_test_temperature"]
...
Notes: Table 3
List: ["yield_strength", "tensile_strength", "elongation", "hardness", "impact_energy", "charpy_impact_test_temperature"]
...
| % EL | 17±1 | 14±3 | 16±1 |  |  |  |  |
Notes: Table 2:
List: ["yield_strength", "tensile_strength", "elongation", "welding_process", "etc"]
...
PROPERTY_EXTRACT = """Use only explicitly reported welding experimental data, including base-material controls.
...
Retain source values, units, ranges, uncertainties and inequalities without averaging or conversion.
J/cm2 is not J. Leave units empty when absent from the supplied table/caption/notes.
Exclude specification limits from measured results. Do not turn missing-value dashes into zeros.
A value repeated only because a merged cell was expanded is one group-level value, not replicates.
Preserve genuinely repeated measurements and label reported averages separately.
Side-by-side table blocks may be independent: do not associate rows solely by visual alignment.
An example marked as an excerpt only covers its displayed rows. Extract the full supplied input.
Follow the requested property list for the current input, not the property list of an example.
...
Extract {{ prop }} from this welding table, retaining row/column associations.
{{ format }}
Examples:
{{ examples }}
Return only a list of dictionaries with "meta", selected property lists.
Input: {{ paragraph }}
"""
